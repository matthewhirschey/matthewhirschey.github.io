...
**Note on `&` in YAML:** the Lua filter's `json_escape()` emits `&` (Unicode escape) for `&` so JSON-LD survives Pandoc's HTML escaping and the browser's `</script>` scan. You can use `&` in titles and venues without breaking JSON parse — the visible HTML will render `&amp;` (correct HTML escaping). Caveat: literal `<` and `>` are *stripped by Pandoc's YAML parser* before any filter runs (parsed as bare HTML tags). If you ever need literal angle brackets in a title, escape them as `\<` / `\>` in YAML or use `&lt;` / `&gt;`.
